...
router.get('/delete/:id', passport.csrfProtection, (req, res) => {
    lists.delete(req.params.id, (err, deleted) => {
        if (err) {
    ...
    lists.filter(req.body, Number(req.query.parent) || false, (err, data, total, filteredTotal) => {
        ...
        res.json({
            ...
            data: data.map((row, i) => [
                ...
                // ' <a href="/lists/view/' + row.id + '">' + htmlescape(row.name || '') + '</a>',
                htmlescape(row.name || ''),
                row.cid,
                row.subscribers,
                htmlescape(striptags(row.description) || ''),
                // '<a href="/lists/delete/' + row.id + '">' + _('Delete') + '</a> &nbsp;&nbsp;<a href="/lists/edit/' + row.id + '">' + _('Edit') + '</a>' ]
                row.id ]
            )
        });
    });
